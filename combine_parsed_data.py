...
def determine_crime_classification():
    """
    Helper -- Used in support of making the crime_equiv.csv
    """
...
def make_crime_remap_lookup():
    """
    Loads the crime_equiv.csv into a dictionary and returns.

    Accessing on [city][crime_type] returns the Memphis equivalent crime_type or None.
    """

    crime_equiv_df = pd.read_csv("helper_data/crime_equiv.csv")
...
def combine_parsed_dfs_daily(remap_lu: dict):
    """
    Ended up not being used, but this method was originally used to combine the individual parsed crime data and the
    COVID case data in terms of absolute counts on a daily interval.
    """
...
def normalize_and_combine_single_city(case_df: pd.DataFrame, crime_df: pd.DataFrame, city_name: str,
                                      crime_equiv_lookup: dict):
    """
    The primary function of this file, takes in city specific daily crime data and COVID cases, outputs a
    file containing the normalized representation of that data on a monthly interval using Memphis equivalent crime
    types.
    """
    ...
    # Merge COVID Cases and Crime
    combined_df = crime_df.merge(sub_case_df, on=["date", "city"])

    # Get list of crime types by removing columns that we know do not represent crime types.
    ...
    # Remove Dates After 2021 (Dates before 2018 removed in earlier step)
    combined_df = combined_df[combined_df["date"] < datetime.datetime.strptime("01/01/2022", "%m/%d/%Y")]
    ...
    # Remap Crime Types To Memphis Equivalents. Store in New DataFrame.
    ...
    for i, row in combined_df.iterrows():
        ...
        for native_crime_type in city_crime_types:
            ...
            except KeyError:  # This discards crime types that are unable to map to Memphis Crime Types
                continue
    ...
    # Perform Min/Max Normalization on the crime_equivalent, monthly crime and covid counts.
